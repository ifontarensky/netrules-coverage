...
class ColorPrint:
    ...
    @staticmethod
    def _write(message, end = '\n'):
        sys.stdout.write(message + end)

    @staticmethod
    def _format_arg(k, v):
        return Color.F_Cyan+"%s"%k+Color.F_Default+"=%s"%v
    ...
    @staticmethod
    def print_plugin(name, args, end = '\n'):
        param = ', '.join([ColorPrint._format_arg(k, v) for k,v in args.items()])
        message = "[+] "+Formatting.Bold + Color.F_Blue + name +'(' + param + ')'+ Color.F_Default +Formatting.Reset
        ColorPrint._write(message, end)

    @staticmethod
    def print_plugin_disable(name, args, end = '\n'):
        param = ', '.join(["%s=%s"%(k, v) for k,v in args.items()])
        message = "[-] "+Color.F_LightGray + name +'(' + param + ')'+ Color.F_Default
        ColorPrint._write(message, end)
